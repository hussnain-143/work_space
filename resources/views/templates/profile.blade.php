                        ...
                        <div class="container mt-4 d-flex justify-content-center">
                            ...
                            <div class="card shadow-lg border-0 p-4 text-center profile-card">
                                <div class="d-flex justify-content-center">
                                    <img src="{{ Auth::user()->image ? asset('images/' . Auth::user()->image) : asset('images/default-user.png') }}"
                                        alt="Profile Image" class="rounded-circle shadow-lg profile-img">
                                ...
                                <div class="mt-3 profile-details">
                                    <p><i class="bi bi-telephone-fill text-info"></i> <strong>Phone:</strong>
                                        {{ Auth::user()->phone ?? 'Not Provided' }}</p>
                                    <p><i class="bi bi-briefcase-fill text-warning"></i> <strong>Position:</strong>
                                        {{ ucfirst(Auth::user()->role) }}</p>
                                </div>
                                <a href="{{ route('user.edit', Auth::user()->id) }}"
                                    class="btn btn-color text-white">Update</a>
                            </div>
                        </div>
